...
class Activity extends Component {
    ...
    getActivity(orgData) {
        ...
        NetInfo.isConnected.fetch().then(isConnected => {
            if (isConnected) {
                ...
                this.props.getActivityDetails(inputData).then(() => {
                    console.log(TAG, "getActivityDetails::" + JSON.stringify(this.props.activityData))
                });
            } else {
    ...
    render() {
        const { activityData, loading, errorMsg } = this.props

        return (
            <View style={stylesConfig.container}>
                <HeaderWithBackImage title={appStrings.activities} />
                {activityData.length > 0 ? <View style={{ flex: 1, }}>
                    {activityData.length == 0 ? <EmptyView msg={appStrings.noRecordFound} /> : <View style={{ margin: 10 }}>
                        <FlatList
...
const mapStateToProps = state => {
    return {
        ...
        errorMsg: state.activityReducer.errorMsg,
        activityData: state.activityReducer.activityData,
