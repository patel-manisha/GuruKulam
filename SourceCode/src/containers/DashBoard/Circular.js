...
class Circular extends Component {
    ...
    componentDidMount() {
        ...
        asyncStorage.getOrganizationData().then((data) => {
            ...
            this.getCircularDetails(orgData);
        })

    }
    getCircularDetails(orgData) {
        ...
        NetInfo.isConnected.fetch().then(isConnected => {
            if (isConnected) {  
                ...
                this.props.getCircularsDetails(inputData).then(() => {
    ...
    render() {
        const { circularData, loading, errorMsg } = this.props
        let tempArray = [];
        return (
            <View style={stylesConfig.container}>
                <HeaderWithBackImage title={appStrings.circular} />
                {circularData.length > 0 ? <View style={{ flex: 1, }}>
                    {circularData.length == 0 ? <EmptyView msg={appStrings.noRecordFound} /> : <View style={{ margin: 10 }}>
                        <FlatList
...
const mapStateToProps = state => {
    return {
        loading: state.circularsReducer.isLoading,
        errorMsg: state.circularsReducer.errorMsg,
        circularData: state.circularsReducer.circularData,
